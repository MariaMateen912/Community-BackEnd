...
router.post('/saveAttendance', async (req, res) => {
  try {
    ...
    for (const noticeId in attendance) {
      await Notice.findByIdAndUpdate(noticeId, { attendance: attendance[noticeId] });
    }
...
router.get('/attendance/:meetingId', async (req, res) => {
  try {
    ...
    const attendingCount = meeting.attendance.filter(a => a === 'Attending').length;
    const notAttendingCount = meeting.attendance.filter(a => a === 'Not Attending').length;
...
router.post(
  ...
  console.log('test')
    ...
    const result = await Event.findOne({ _id: ObjectId(id) });
    if(result)
    {
      console.log(result)
    }
